...
EnvItem cenarioTeste[] = {
        {{0, 0, TAMANHO_X_CENARIO_1, TAMANHO_Y_CENARIO_1}, 0, SKYBLUE},//background
...
int main()
{
    ...
    while (!WindowShouldClose())
    {
       ...
       Texture2D cenario_fundo = LoadTexture("cenario/leitecondensado.jpg.png");
        DrawTextureRec(cenario_fundo, cenarioTeste[0].retangulo, (Vector2){0, 0}, RAYWHITE);
       
       //impressão do piso
        Texture2D cinza = LoadTexture("cenario/chao_mario.png");
        DrawTextureRec(cinza, cenarioTeste[1].retangulo, (Vector2){0, 550}, RAYWHITE);
        Texture2D rosa = LoadTexture("cenario/chao_mario.png");
        DrawTextureRec(rosa, cenarioTeste[2].retangulo, (Vector2){560, 550}, RAYWHITE);
        Texture2D marrom = LoadTexture("cenario/chao_mario.png");
        DrawTextureRec(marrom, cenarioTeste[3].retangulo, (Vector2){1900, 550}, RAYWHITE);
        //impressão dos demais blocos
        Texture2D verde = LoadTexture("cenario/chao_mario.png");
        DrawTextureRec(verde, cenarioTeste[4].retangulo, (Vector2){850, 480}, RAYWHITE);
        Texture2D azul = LoadTexture("cenario/chao_mario.png");
        DrawTextureRec(azul, cenarioTeste[6].retangulo, (Vector2){960, 410}, RAYWHITE);
        Texture2D amarelo = LoadTexture("cenario/chao_mario.png");
        DrawTextureRec(amarelo, cenarioTeste[5].retangulo, (Vector2){1600, 650}, RAYWHITE);
        Texture2D preto = LoadTexture("cenario/bloco_mario.png");
        DrawTextureRec(preto, cenarioTeste[7].retangulo, (Vector2){960, 150}, RAYWHITE);
        Texture2D laranja = LoadTexture("cenario/Bandeira.png");
        DrawTextureRec(laranja, cenarioTeste[8].retangulo, (Vector2){2520, 350}, RAYWHITE);
        ...
        UnloadTexture(cinza);
        UnloadTexture(rosa); 
        UnloadTexture(marrom);
        UnloadTexture(verde);
        UnloadTexture(azul);
        UnloadTexture(amarelo);
        UnloadTexture(preto);
...
{  
    ...
    if ((jogador->posicao.x + TAMANHO_X_JOGADOR / 2) > TAMANHO_X_CENARIO_1)
    {
        jogador->posicao.x = TAMANHO_X_CENARIO_1 - TAMANHO_X_JOGADOR / 2; //Limites para direita
    } else if (jogador->posicao.x < TAMANHO_X_JOGADOR / 2)
    {
        jogador->posicao.x = TAMANHO_X_JOGADOR / 2; //Limites para a esquerda
    }
    
    if ((jogador->posicao.y) > TAMANHO_Y_CENARIO_1)  //Limites na vertical
    {
        jogador->posicao.y = TAMANHO_Y_CENARIO_1; 
...
void UpdateInimigos(Inimigo *inimigo, EnvItem *envItems, int tamanhoInimigos, int envItemsLength, float delta)
{
    for (int i = 0; i < tamanhoInimigos; i++)
    {
        ...
        if ((inimigo->posicao.x + TAMANHO_MINION_X / 2) > TAMANHO_X_CENARIO_1)
        {
            inimigo->posicao.x = TAMANHO_X_CENARIO_1 - TAMANHO_MINION_X / 2; //Limites para direita
